...
const SetWalletConfigModal = ({ taskName, configSchema, show, onHide, confirm }) => {
        ...
        const rowList = Object.keys(configSchema || {}).map((key) => {
            if ((configSchema[key] || {}).type === 'input') {
                return [
                    ...
                    { type: 'input', colWidth: 6, key, value: record[key], placeholder: t('taskManage.inputPlaceholder', { field: key, defaultValue: `请输入${key}` }), style: { fontSize: '1.0vw' } }
                ];
            }
    ...
    const handleConfirm = () => {
        const payload = { ...config, mode: configScope };
        confirm(payload);
    };
